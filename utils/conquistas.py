...
async def atualizar_conquistas(user_data, carta, user_id):
    ...
    if count_raridade in [5, 10, 25, 50]:
        mensagens.append(f"🏆 <@{user_id}> colecionou {count_raridade} cartas **{raridade}**!")
    ...
    if count_era in [5, 10, 25, 50]:
        mensagens.append(f"🏆 <@{user_id}> colecionou {count_era} cartas da era **{era}**!")
    ...
    if count_grupo in [5, 10, 25, 50]:
        mensagens.append(f"🏆 <@{user_id}> colecionou {count_grupo} cartas do grupo **{grupo}**!")
